...
function App() {
  ...
  const handleStartAgent = () => {
    ...
    try {
      const runtime = new wasmModule.AgentWasmRuntime()
          ...
          <div>
            ...
            <div style={{
              ...
              {chatMessages.map(msg => (
                <div key={msg.id} style={{
                  ...
                  padding: '8px',
                  borderRadius: '4px',
                  backgroundColor: msg.sender === 'user' ? '#e3f2fd' :
                    msg.sender === 'agent' ? '#e8f5e8' : '#fff3e0'
                }}>
                  <div style={{
                    fontSize: '12px',
                    color: '#666',
                    ...
                    {msg.sender === 'user' ? '👤 You' :
                      msg.sender === 'agent' ? '🤖 Agent' : '⚙️ System'} • {msg.timestamp}
                  </div>
                  <div>{msg.content}</div>
                </div>
              ))}
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button
                ...
                style={{
                  backgroundColor: !inputMessage.trim() || loading ? '#888' : '#4CAF50',
                  color: 'white',
                  border: 'none',
                  padding: '10px 20px',
                  borderRadius: '4px',
            ...
            <p style={{ marginTop: '20px', fontSize: '12px', color: '#666' }}>
              Agent Status: {agentRunning && agentRuntime?.is_running() ? '🟢 Running' : '🔴 Stopped'}
            </p>
          </div>
        )}
      </div>
